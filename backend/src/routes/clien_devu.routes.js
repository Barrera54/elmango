...
router.get("/", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("SELECT * FROM 	cliente_deudor");
    res.json(rows);
  } catch (error) {
...
router.get("/:ID_produ", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT nombre, precio FROM cliente_deudor WHERE ID_produ = ?",
      [req.params.ID_produ]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO cliente_deudor (nombre, descripcion, precio) values (?, ?, ?)",
...
router.put("/:ID_produ", async (req, res) => {
  ...
  try {
    // Se cambia 'id' por 'ID_produ' en la consulta SQL
    const [result] = await pool.query(
      "UPDATE cliente_deudor SET nombre = ?, descripcion = ?, precio = ? WHERE ID_produ = ?",
