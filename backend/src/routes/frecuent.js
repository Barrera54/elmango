...
router.get("/", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("SELECT * FROM cliente_frecuent");
    res.json(rows);
  } catch (error) {
...
router.get("/:ID_produ", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT nombre, precio FROM cliente_frecuent   WHERE ID_produ = ?",
      [req.params.ID_produ]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO cliente_frecuent (nombre, descripcion, precio) values (?, ?, ?)",
...
router.put("/:ID_produ", async (req, res) => {
  ...
  try {
    // Se cambia 'id' por 'ID_produ' en la consulta SQL
    const [result] = await pool.query(
      "UPDATE cliente_frecuent SET nombre = ?, descripcion = ?, precio = ? WHERE ID_produ = ?",
...
router.delete("/:ID_produ", async (req, res) => {
  try {
    // Se cambia 'id' por 'ID_produ' en la consulta SQL
    const [result] = await pool.query(
      "DELETE FROM cliente_frecuent WHERE ID_produ = ?",
      [req.params.ID_produ]
    );
    if (result.affectedRows === 0)
      return res.status(404).json({ error: "Producto no encontrado" });
